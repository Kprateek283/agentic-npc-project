...
- [x] **C4. Semantic response cache**: embed the incoming lore question, cosine-match against recent questions per NPC, and on a match above a tuned threshold (0.90) serve the stored answer with no LLM call. In-process (dict + O(n) scan), not Redis — the service is single-process and lore is static, so a dict does what a Redis KV would without a new dependency (upgrade path = Redis vector search, noted in a `ponytail:` comment). Only near-neutral emotional contexts are cached. *Verified live + committed benchmark: 100% hit rate on repeats, hit median 65 ms vs LLM miss median 20.8 s (~320× faster); eval suite runs clean with the cache enabled (0 errors).* Threshold tuned empirically so same-topic/different-intent questions never false-hit.
- [~] **C5. LangSmith tracing** (already in requirements): env-gated (off by default), documented in `docs/langsmith.md` + `.env.example`. *Verified by pointing the LangSmith SDK at a local capture server: the exported runs contain the full RAG chain (RunnableSequence → ChatPromptTemplate → VectorStoreRetriever → ChatOllama → StrOutputParser) and the multi-step agent loop with tool executions (LangGraph, call_model, should_continue, lore_book_search, quest_status).* **Remaining (user step):** the two README UI screenshots need a logged-in LangSmith account — can't be authored here, so this stays unticked until captured (`docs/img/langsmith_rag.png`, `langsmith_agent.png`).
