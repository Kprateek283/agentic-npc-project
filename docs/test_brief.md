...
- 2026-09-24 — Python 2, CLI-backed provider: done. `tests/test_agy_chat_contract.py`, 20 cases with `subprocess.run` faked: a metacharacter-laden prompt as one argv element with no shell (whole command asserted literally), `--model` only for a non-empty model and last, subprocess timeout = CLI timeout + 30 s, non-zero exit / unparsable output / timeout / missing binary each raising, stderr cut to 500, `bind_tools` failing when the quest agent is built, streaming in one piece. No production change. Mutation-checked 14 breaks, all went red. Findings: (a) exit 0 with `{"response": ""}` returns an empty reply instead of raising; (b) a null or non-string response raises a pydantic `ValidationError` rather than `RuntimeError`. Full results: `docs/test_results/python-02-agy-provider.md`. Next: Python 3, provider selection.
- 2026-09-24 — Python 3, provider selection: done. `tests/test_config.py`, 23 cases reloading `config` under clean environments (`load_dotenv` disabled, Qdrant faked, restored to ollama once at the end): gemini's shared pinned model and key requirement, ollama's two models and overrides, agy's CLI-light/Ollama-heavy pair and reported names, case-insensitive names, unknown providers and vector stores failing at import, embeddings on Ollama in every mode. No production change. Mutation-checked 17 breaks, all went red. The file takes ~7 s here because each reload builds TLS contexts. Findings: (a) consumers bind `llm_light`/`llm_heavy`/`embeddings` at import, so a reload does not reach them; (b) `LLM_PROVIDER` is not stripped; (c) bad integers fail with a message that does not name the variable. Full results: `docs/test_results/python-03-provider-selection.md`. Next: Python 4, the prompt templates.
- 2026-09-24 — Python 4, prompt templates: done. `tests/test_prompt_templates.py`, 33 cases: placeholders match what each chain supplies, no stray braces, the RAG template rendered through `build_rag_chain` for all eight real personas and the event template captured by a fake model through `graph_builder`'s prompt callable, every value in place with none left unfilled, and sixteen deliberate rule sentences (grounding, 0.5 threshold, general mood never the speaker's fault, event-name search, apology marker, no Thought:) asserted with whitespace collapsed. No production change. Mutation-checked 15 breaks, all went red. Findings: (a) BUG: persona text is not escaped before joining the RAG template, so a `{word}` in any summary or backstory fact makes every lore question to that NPC raise KeyError (no current gamedata has one); (b) the RAG template never explains the "after apologising" marker. Full results: `docs/test_results/python-04-prompt-templates.md`. Next: Python 5, the agent wrapper.
- 2026-09-24 — Python 5, agent wrapper: done. `tests/test_npc_agent.py`, 12 cases on a real `NpcAgent` built from Elara's persona with a recording fake chat model, fixed embeddings and a fixed retriever: cache hits short-circuit the model, speakers are never cached or embedded, streaming yields pieces then stores the whole answer, a cancelled stream stores nothing, the quest agent's words and content blocks, and the iteration cap returning the in-character line after six model calls. No production change. Mutation-checked 12 breaks, all went red. Findings: (a) a stream of textless chunks caches "" and replays it for an hour; (b) the cap's fallback line starts "Forgive me, my thoughts wandered", the brief's trap. Full results: `docs/test_results/python-05-agent-wrapper.md`. Next: Python 6, the REST surface.
- 2026-09-24 — Python 6, REST surface: done. `tests/test_rest_api.py`, 15 cases through the real app, router and agent registry with fake agents via `TestClient`: `/v1/npcs` lists what is loaded, unknown NPC 404, bad event type 422 (before the NPC check, never reaching the agent), malformed bodies 422, provider failure 502 with only the error type (no trace, path or secret), chat and event routing with their contexts, legacy keys. No production change. Mutation-checked 13 breaks, all went red. Findings: (a) REST silently drops a speaker, so every REST request is anonymous; (b) every exception, including programming errors, is reported as "AI provider call failed". Full results: `docs/test_results/python-06-rest-surface.md`. Next: Python 7, the lore tool.
- 2026-09-24 — Python 7, lore tool: done. `tests/test_lore_tool.py`, 27 cases with fake embeddings, in-process FAISS and a recording Qdrant fake: unreadable, empty or fact-less lore gives no tool, the collection name is `lore_<npc directory>`, the Qdrant branch recreates that per-NPC collection with the configured URL and k, k facts are returned, the tool's name and newline-joined answer, long facts chunked, all eight real lore files indexed whole. No production change. Mutation-checked 12 breaks, all went red. Findings: (a) valid JSON of the wrong shape (a list, a non-string fact) raises, so that NPC is skipped at startup instead of just losing its lore tool; (b) a string `known_facts` is indexed per character; (c) a lore file outside an NPC directory gets collection `lore_`. Both lists are now complete; the brief's not-worth-testing packages get no tests by design. Full results: `docs/test_results/python-07-lore-tool.md`.
