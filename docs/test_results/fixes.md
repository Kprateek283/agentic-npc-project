...
- [ ] python-03 #1 the existing reload test is the red one — OPEN: the same finding as python-01 #1, seen from the other side. `test_agy_chat.py::test_config_with_agy_provider` deletes `LLM_PROVIDER` in teardown and reloads into the gemini default; the new `test_config.py` shows the working pattern (reset to `ollama` before the final reload) but the old test is untouched. The defect is in a test, and tests are not mine to change.
