...
1. **Go Orchestrator (The Dungeon Master)**:
   ...
   - **State Machine & Logic**: Handles quest lifecycles, player inventories, NPC relationships, and episode-based memories from which emotions and trust are computed.
...
2. Go backend validates the player and target NPC, records the event as an episode row in PostgreSQL, and computes current feelings (speaker emotions and general mood) and ranked memory lines from stored episodes.
3. Go backend calls the Python AI service via gRPC (`Think` method), passing the question, speaker emotions, general mood, and ranked memory lines.
...
- **`context_formatter.py`**: Helper script to format the dynamic context (speaker emotions, general mood, ranked memory lines, quest progress) into string blocks injected into the LLM prompt.
...
- **`internal/app/app.go`**: The Dependency Injection hub. It initializes environment variables, database connections (PostgreSQL and Redis), seeds the DB with initial gamedata, loads event rules (`rules.Load`), initializes `QuestManager`, and wires up the HTTP server and gRPC client.
...
- **`handlers/game_handler.go`**: The primary logic flow for game events. It does the following:
  1. Records the event as an episode row in PostgreSQL (tracking actor, delta, intensity, count, forgiveness, and betrayal state).
  2. Emits an `EMOTIONS` WebSocket frame with computed feelings toward the player and general mood.
  3. Calls `QuestManager` to validate preconditions and process quest progression.
  4. Computes feelings toward the player and general mood, plus ranked memory lines via `npcstate` and `memory`.
  5. Triggers a gRPC call to the Python service with the computed context (speaker emotions, general mood, ranked memory lines, quest state).
...
- **`memory/`**: Pure mathematical domain logic for emotion computation, escalation on repeated harm, diminishing returns on positive events, intensity-based half-life decay, forgiveness, and betrayal penalties.
- **`rules/`**: Loads and validates `gamedata/events.json`, producing the runtime `memory.Config` and event definitions.
- **`npcstate/`**: Converts database memory rows into `memory.Episode` slices, computing current feelings toward a specific actor or general mood, and computing player-NPC trust.
...
- **`infra/grpc_client/ai_client.go`**: A wrapper around the generated gRPC client that creates and formats the `EventRequest` sent to the Python microservice.
...
- **`gamedata/`**: A crucial directory containing the static JSON content that dictates game state and behavior. It is read by both the Go and Python components:
  ...
  - `events.json`: Defines event emotion deltas, intensity, harmful/apology/conversation flags, and tuning knobs for escalation, decay, forgiveness, and mood sharing (read by Go).
- **`proto/ai.proto`**: The Protocol Buffer schema file defining the contract between the Go Orchestrator and the Python AI Service. It specifies the `AIBrain` service and the structure of `EventRequest` and `ActionResponse`.
