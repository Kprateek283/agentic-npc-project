...
func (_q *ItemQuery) QueryInventoryItems() *InventoryItemQuery {
...
func (_q *ItemQuery) WithInventoryItems(opts ...func(*InventoryItemQuery)) *ItemQuery {
	query := (&InventoryItemClient{config: _q.config}).Query()
...
func (_q *ItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Item, error) {
	...
	if query := _q.withInventoryItems; query != nil {
		if err := _q.loadInventoryItems(ctx, query, nodes,
			func(n *Item) { n.Edges.InventoryItems = []*InventoryItem{} },
			func(n *Item, e *InventoryItem) { n.Edges.InventoryItems = append(n.Edges.InventoryItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ItemQuery) loadInventoryItems(ctx context.Context, query *InventoryItemQuery, nodes []*Item, init func(*Item), assign func(*Item, *InventoryItem)) error {
	...
	query.Where(predicate.InventoryItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(item.InventoryItemsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.item_inventory_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "item_inventory_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "item_inventory_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
