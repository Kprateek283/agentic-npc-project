...
type PlayerQuery struct {
	...
	withQuestStates      *PlayerQuestStateQuery
	withInventory        *InventoryItemQuery
	withNpcRelationships *PlayerNPCRelationshipQuery
...
// QueryQuestStates chains the current query on the "quest_states" edge.
func (_q *PlayerQuery) QueryQuestStates() *PlayerQuestStateQuery {
	query := (&PlayerQuestStateClient{config: _q.config}).Query()
...
func (_q *PlayerQuery) QueryInventory() *InventoryItemQuery {
...
// QueryNpcRelationships chains the current query on the "npc_relationships" edge.
func (_q *PlayerQuery) QueryNpcRelationships() *PlayerNPCRelationshipQuery {
...
func (_q *PlayerQuery) Clone() *PlayerQuery {
	...
	return &PlayerQuery{
		...
		withQuestStates:      _q.withQuestStates.Clone(),
		withInventory:        _q.withInventory.Clone(),
		withNpcRelationships: _q.withNpcRelationships.Clone(),
...
// WithQuestStates tells the query-builder to eager-load the nodes that are connected to
// the "quest_states" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PlayerQuery) WithQuestStates(opts ...func(*PlayerQuestStateQuery)) *PlayerQuery {
	query := (&PlayerQuestStateClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withQuestStates = query
	return _q
}
...
func (_q *PlayerQuery) WithInventory(opts ...func(*InventoryItemQuery)) *PlayerQuery {
	query := (&InventoryItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithNpcRelationships tells the query-builder to eager-load the nodes that are connected to
// the "npc_relationships" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PlayerQuery) WithNpcRelationships(opts ...func(*PlayerNPCRelationshipQuery)) *PlayerQuery {
	query := (&PlayerNPCRelationshipClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withNpcRelationships = query
	return _q
}
...
func (_q *PlayerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Player, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [3]bool{
			_q.withQuestStates != nil,
			_q.withInventory != nil,
			_q.withNpcRelationships != nil,
		}
	)
	...
	if query := _q.withQuestStates; query != nil {
		if err := _q.loadQuestStates(ctx, query, nodes,
			func(n *Player) { n.Edges.QuestStates = []*PlayerQuestState{} },
			func(n *Player, e *PlayerQuestState) { n.Edges.QuestStates = append(n.Edges.QuestStates, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withInventory; query != nil {
		if err := _q.loadInventory(ctx, query, nodes,
			func(n *Player) { n.Edges.Inventory = []*InventoryItem{} },
			func(n *Player, e *InventoryItem) { n.Edges.Inventory = append(n.Edges.Inventory, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withNpcRelationships; query != nil {
		if err := _q.loadNpcRelationships(ctx, query, nodes,
			func(n *Player) { n.Edges.NpcRelationships = []*PlayerNPCRelationship{} },
			func(n *Player, e *PlayerNPCRelationship) {
				n.Edges.NpcRelationships = append(n.Edges.NpcRelationships, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *PlayerQuery) loadQuestStates(ctx context.Context, query *PlayerQuestStateQuery, nodes []*Player, init func(*Player), assign func(*Player, *PlayerQuestState)) error {
	...
	query.withFKs = true
	query.Where(predicate.PlayerQuestState(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(player.QuestStatesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.player_quest_states
		if fk == nil {
			return fmt.Errorf(`foreign-key "player_quest_states" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "player_quest_states" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *PlayerQuery) loadInventory(ctx context.Context, query *InventoryItemQuery, nodes []*Player, init func(*Player), assign func(*Player, *InventoryItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Player)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.InventoryItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(player.InventoryColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.player_inventory
		if fk == nil {
			return fmt.Errorf(`foreign-key "player_inventory" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "player_inventory" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *PlayerQuery) loadNpcRelationships(ctx context.Context, query *PlayerNPCRelationshipQuery, nodes []*Player, init func(*Player), assign func(*Player, *PlayerNPCRelationship)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Player)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.PlayerNPCRelationship(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(player.NpcRelationshipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.player_npc_relationships
		if fk == nil {
			return fmt.Errorf(`foreign-key "player_npc_relationships" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "player_npc_relationships" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
