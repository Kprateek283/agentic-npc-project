...
type QuestQuery struct {
	...
	withPlayerQuestStates *PlayerQuestStateQuery
...
// QueryPlayerQuestStates chains the current query on the "player_quest_states" edge.
func (_q *QuestQuery) QueryPlayerQuestStates() *PlayerQuestStateQuery {
...
func (_q *QuestQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *QuestQuery) FirstIDX(ctx context.Context) string {
...
func (_q *QuestQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *QuestQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *QuestQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *QuestQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *QuestQuery) Clone() *QuestQuery {
	...
	return &QuestQuery{
		...
		withPlayerQuestStates: _q.withPlayerQuestStates.Clone(),
...
// WithPlayerQuestStates tells the query-builder to eager-load the nodes that are connected to
// the "player_quest_states" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *QuestQuery) WithPlayerQuestStates(opts ...func(*PlayerQuestStateQuery)) *QuestQuery {
	query := (&PlayerQuestStateClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPlayerQuestStates = query
	return _q
}
...
func (_q *QuestQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Quest, error) {
	...
	if query := _q.withPlayerQuestStates; query != nil {
		if err := _q.loadPlayerQuestStates(ctx, query, nodes,
			func(n *Quest) { n.Edges.PlayerQuestStates = []*PlayerQuestState{} },
			func(n *Quest, e *PlayerQuestState) { n.Edges.PlayerQuestStates = append(n.Edges.PlayerQuestStates, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *QuestQuery) loadPlayerQuestStates(ctx context.Context, query *PlayerQuestStateQuery, nodes []*Quest, init func(*Quest), assign func(*Quest, *PlayerQuestState)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Quest)
	for i := range nodes {
	...
	query.Where(predicate.PlayerQuestState(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(quest.PlayerQuestStatesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.quest_player_quest_states
		if fk == nil {
			return fmt.Errorf(`foreign-key "quest_player_quest_states" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "quest_player_quest_states" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *QuestQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(quest.Table, quest.Columns, sqlgraph.NewFieldSpec(quest.FieldID, field.TypeString))
	_spec.From = _q.sql
