...
2. Open `index.html` in a browser (double-click, or `python -m http.server` in this folder). If the page is served over HTTP instead of opened from disk, its origin (e.g. `http://localhost:5500`) must be added to `ALLOWED_ORIGINS`.
